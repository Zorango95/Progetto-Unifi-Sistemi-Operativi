...
/*VARIABILI GLOBALI*/
	...
	struct T_structure *head = NULL;/*puntatore che conterrÃ  la testa dello scheduler*/
	struct T_structure *newhead = NULL;/*puntatore d'appoggio che conterrÃ  la nuova testa*/
	struct T_structure *tail = NULL;/*puntatore che conterrÃ  la coda dello scheduler*/
	int sch = 0; /*variabile con la quale specifico se l'ordinamento deve essere per esecuzioni rimaste o per prioritÃ (di default)*/
	int n= 0;/*variabile con la quale il programma capisce che lo scheduler Ã¨ vuoto*/
...
void switch_two(LIST a, LIST b, LIST c){
	...
	if(a == head && b == a){/*verifico se l'elemento in  testa Ã¨ il piÃ¹ piccolo, in caso negativo esegue lo switch tra i primi due task */
	...
	}else if(a == head && b != a){/*verifico se l'elemento in  testa Ã¨ il piÃ¹ piccolo, in caso negativo esegue lo switch tra la testa e l'elemento individuato piÃ¹ piccolo */
	...
	}else if(a != head && a==b){/*quando la lista contiene giÃ  alcuni elementi in ordine, l'ordinamento continuerÃ  dall'elemento successivo (all'ultimo, che Ã¨ giÃ  nella 						posizione finale), quindi l'elemento precedente corrisponde all'elemento puntato*/
	...
	}else if(a != head && a!= b){/*quando la testa contiene giÃ  l'elemento piÃ¹ piccolo/maggiore, l'ordinamento inizia da un task successivo a quello iniziale*/
...
int minimo_execution(LIST point, LIST precPoint){
	...
	int min = point -> execution; /* assegnazione ad una variabile, l'esecuzione dell'elemento puntato, il possibile elemento con prioritÃ  minima*/
	...
	while(cont2 != NULL){
		/*controllo se il numero delle esecuzioni del task successivo Ã¨ minore al valore min*/
		if(cont2 -> execution <= min){
			...
			c=c+1; /*incremento il contatore degli spostamento poichÃ¨ ho eseguito lo spostamento*/
	...
	if(c!=0 && min != minore){/*se Ã¨ stato riscontrato uno spostamento e il minimo ritrovato Ã¨ diverso da quello della testa, si chiama la procedura di switch tra la testa da dove iniziare l'ordinamento (point), elemento precedente al minimo(prec), puntatore della struttura dalla quale si inizia l'ordinamento(cont5)*/
...
/*FUNZIONE CHE ORDINA IN BASE ALLA PRIORITÃ€ MASSIMA*/
...
int massima_priority(LIST point, LIST precPoint){
	...
	int max = point -> priority; /* assegnazione ad una variabile, la prioritÃ  dell'elemento puntato, il possibile elemento con prioritÃ  massima*/
	LIST succ = point -> next; /* puntatore per l'elemento successivo, usate per non modificare la struttura della lista*/

	int c=0; /* contatore spostamenti, che individua se ci deve effettuare uno spostamento tra i task*/
	int massimo = max;/*variabile d'appoggio, contiene la prioritÃ  del primo elemento*/


	while(cont2 != NULL){/* controlla che la lista abbia altri task*/

		if(cont2 -> priority >= max){/*controlla se la prioritÃ  dell'elemento successivo Ã¨ maggiore di quello precedente, in caso affermativo effettua la scambio dei valori 							d'appoggio per il futuro switch, altrimenti lascia tutto invariato*/
			...
			c=c+1;/*notifico che si effettuerÃ  uno switch*/
	...
	if(c!=0 && max != massimo){/*se Ã¨ stato riscontrato uno spostamento e il massimo ritrovato Ã¨ diverso da quello della testa, si chiama la procedura di switch tra la testa da dove iniziare l'ordinamento (point), elemento precedente al massimo(prec), puntatore della struttura dalla quale si inizia l'ordinamento(cont5)*/
		switch_two(point, prec, cont5);
		return 1;
	}else if(c!=0 && point -> ID < prec -> next -> ID){/*in caso due prioritÃ  hanno lo stesso valore, si effettua lo switch controllando l'id dei task*/
		switch_two(point, prec, cont5);
		return 1;
	}
...
void ordinamento(struct T_structure *head, int sch){
	...
	if(sch==0){ /*ordinamento prioritÃ  = false*/
		printf("\nORDINAMENTO PER PRIORITA'\n");


		while(cont3 != NULL){/*controlla se la lista dei task ha altri elementi*/

			control = massima_priority(cont3, cont4); /* chiamata alla funzione massima prioritÃ , e passa i valori da confrontare*/
			...
			/* se non si Ã¨ verificato lo switch, passa agli elementi successivi */
				cont4 =cont3;
				cont3 = cont3 -> next;
			}
		...
		tail = cont4; /* cont4 Ã¨ l'ultimo elemento prima di null quindi Ã¨ sicuramente la coda, poichÃ¨ cont4 punterÃ  ad un elemento null*/
...
LIST insert_new_task(int x){
	...
	struct T_structure *new_task = malloc(sizeof(T_structure));/*puntatore a struttura che punta ad una allocazione di memoria*/
		...
		new_task -> next = NULL; /* assegna al nuovo task un puntatore che punta a nulla, poichÃ¨ il task verrÃ  inserita in fonda alla lista dei task*/

		if (head == NULL){/*controlla se la testa sia vuota, serve a identificare se Ã¨ il primo task inserito, in caso affermativo assegna sia alla testa che alla coda il 						nuovo task creato*/
			head = new_task;
			tail = head;
		}else{
	...
	ordinamento(head, sch);	/*chiama la funzione d'ordinamento mandandogli come parametro, la testa e il tipo d'ordinamento (per prioritÃ  o rimanenze )*/
}
...
LIST find_task_by_ID(int inner_ID){
	...
	do{
		...
		}else if(find_struct -> ID != inner_ID && find_struct -> next == NULL){/*se l'ID non Ã¨ presente nella lista dei task, risponde con un messaggio di errore*/
...
void delete_by_ID(int x){
	...
	struct T_structure *tmp_struct = NULL;/*inizializza un puntatore a struttura che verrÃ  utilizzato per il task con l'ID selezionato*/
	...
	if(tmp_struct != NULL){/*controlla se Ã¨ stato trovato l'ID ricercato*/


		if(tmp_struct == head){/*controlla che l'ID da eliminare corrisponde alla testa*/
			if(tmp_struct == tail){/*controlla se l'ID corrisponde anche alla coda e quindi imposta sia la testa ceh la coda a null in quanto la lista dei task 							diventerÃ  vuota*/
				head = NULL;
				tail = NULL;
			}else if(tmp_struct -> next == tail){/*controlla se l'elemento successivo all'elemento da eliminare (testa),corrisponde all'elemento in coda, in caso affermativo la coda corrisponde alla testa( Ã¨ rimasto un solo task), altrimenti imposta come testa l'elemento seccessivo all'elemento eliminato*/
				head = tail;
			}else{
				head = head -> next;
			}
		}else {/*caso in cui l'elemento da eliminare non fosse quello in testa*/
			while(struct_pointer -> next != tmp_struct){/*scorrimento della lista fino a che l'elemento Ã¨ diverso dall'elemento da eliminare*/
				struct_pointer = struct_pointer -> next;/*passa all'elemento successivo*/

			}if(tmp_struct != tail){/*verifica che l'elemento da eliminare sia diverso dall'elemento in coda, in caso negativo l'elemento precedente punterÃ  a nulla e 							diventerÃ  la nuova coda dei task*/
...
void execute_tail(){
	...
	if(tail -> execution == 1){/*controlla se il numero delle esecuzioni del task in coda Ã¨ uguale a 1, ciÃ² porta all'eliminazione di quest'ultima*/
		int x = (*tail).ID;/*assegnazione ad una variabile l'ID del task in coda */
		printf("Al task in coda rimane una sola esecuzione, dopo la quale sarÃ  eliminato.\n");
...
void execute_by_ID(int x){
	...
	printf("Eseguo task con ID %d.\n", x);
		if(tmp_struct -> execution == 1){/*controllo che verifica se Ã¨ rimasta una sola esecuzione*/
			printf("Al task in coda rimane una sola esecuzione, dopo la quale sarÃ  eliminato.\n");
...
//FUNZIONE CHE CAMBIA LA PRIORITÃ€ DI UN TASK TRAMITE LA SCELTA DI UN ID

void change_priority_by_ID(int x){

	struct T_structure *tmp_struct = NULL;/*inizializzo un puntatore a struttura che verrÃ  utilizzato per il task con l'ID selezionato*/
	tmp_struct = find_task_by_ID(x);/*viene assegnato al puntatore l'ID ottenuto dalla funzione find_task_by_ID*/

	if(tmp_struct != NULL){
		printf("Inserire nuova priorita':\n");
		scanf("%1d", &tmp_struct -> priority);/*riceve da tastiera la nuova prioritÃ  e la sostituisce alla vecchia*/
...
/* Tramite l'assegnazione di un valore compreso tra 0 e 1 questa funzione mi permette di passare da un ordinamento per prioritÃ  (0) a un ordinamento per rimanenze (1)*/
...
int main(void){\
	...
	while(1){
		...
		int ID_search = -1;/*variabile che conterrÃ  il l'id incerito da tastiera per cercare il task*/

		if(head != NULL){/*verifica che la testa sia diversa da 'NULL' condizione in cui lo scheduler Ã¨ vuoto*/
			n=1;
			print_tasks_list();
		...
		if( n == 0){/* controlla se lo scheduler sia vuoto*/
			...
			do{
				...
				scanf("%1d", &command);/*riceve dal tastiera l'esecuzione che dovrÃ  esser eseguita*/
				getchar();/*attesa che l'utente inserisca un carattere da tastiera*/
				if(command == 1){
					printf("sono nel new task\n");
					n=1;
					new_task = insert_new_task(autoincremental_task_ID);  autoincremental_task_ID += 1; /* new_task non serve piÃ¹, quindi insert_task puÃ² 																	tornare void*/
			...
			stampa_menu();
			printf("Inserire un intero corrispondente alle precedenti:\n");
			scanf("%1d", &comando);/*riceve dal tastiera l'esecuzione che dovrÃ  esser eseguita*/
			getchar();/*attesa che l'utente inserisca un carattere da tastiera*/


		switch(comando){

			case 1: new_task = insert_new_task(autoincremental_task_ID);  autoincremental_task_ID += 1; /* new_task non serve piÃ¹, quindi insert_task puÃ² tornare void*/
				break;
